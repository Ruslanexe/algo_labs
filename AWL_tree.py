class Tree:
    ...
    def remove_val(self, node, val) -> Node:
        if node is None:
            return None
        if node.val > val:
            node.left = self.remove_val(node.left, val)
        elif node.val < val:
            node.right = self.remove_val(node.right, val)
        else:
            # One Child case
            ...
            node.right = self.remove_val(node.right, replace.val)
            if not node.is_balanced():
                if node.get_dif() > 0: # L
                    if node.left.get_dif() < 0: # LR
                        node.left = self.rotate_left(node.left)
                    # LL
                    node = self.rotate_right(node)
                else:
                    if node.right.get_dif() > 0: # RL
                        node.right = self.rotate_right(node.right)
                    # RR
                    node = self.rotate_left(node)
        return node
    ...
    def remove(self, val):
        self.root = self.remove_val(self.root, val)
